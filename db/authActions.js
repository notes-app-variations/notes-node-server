...
exports.login = async (req, res) => {
  try {
    const user = await client.db("app_notes").collection("users").findOne({
...
exports.register = async (req, res) => {
  ...
  try {
    await client.db("app_notes").collection("users").findOne({
      email: user.email,
    })
  } catch (err) {
    return res
  ...
  try {
    ...
    const { hash, ...userWithoutHash } = result.ops[0]
